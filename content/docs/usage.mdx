...
Both the [feeds](https://briefkastenhq.com/feeds) as well as [bookmarks](https://briefkastenhq.com/bookmarks) pages have a search input in the nav bar which will do a case-insensitive search on the titles, descriptions and URLs of bookmarks and the title and feed item content (i.e. the article) of RSS items.

## Filtering

Currently only the [feeds](https://briefkastenhq.com/feeds) page has a filtering mechanism. The Bookmarks will have a filtering mechanism to leverage your tags and categories soon too. Stay tuned!
...
To manager your RSS feeds in Briefkasten, go to ["Settings" -> "Feeds"](https://briefkastenhq.com/settings?tab=feeds). There you can add, edit, or delete RSS feed URLs.

When you add a new feed URL, we will initially fetch the last 10 items to add to your list. We will then queue up a background task to fetch new items every hour. To view new items, you can check out the Dashboard "New Items" section(s), or go to the [feeds page](https://briefkastenhq.com/feeds) directly. The feeds page will auto-update when tab away, and back to it, to show you the latest items. No manual refreshing necessary.

## Import / Export

Briefkasten allows you to easily import and export your bookmarks. Visit ["Settings" -> "User"](https://briefkastenhq.com/settings?tab=user) and find the "Import / Export" section.
...
<Callout className="shadow-xs">
  ...
  [existing
  parsers](https://github.com/ndom91/briefkasten/blob/main/apps/web/src/routes/settings/import/pocket.ts)
  for examples.
</Callout>
...
<Screenshot src={QuickAdd} alt="Quick Add" className="shadow-xs" />
...
<Screenshot src={DragNDrop} alt="Drag and Drop Add" className="shadow-xs" />
