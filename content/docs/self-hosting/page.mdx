...
<Callout icon="🚀" type="info" title="Briefkasten v2">
  ...
  `v2` soon. The below instructions are for v1 and will be updated shortly.
</Callout>
...
<Callout>
  The image uploading functionality will be skipped over entirely if you do not
  pass a `SUPABASE_URL` environment variable. No other action is necessary.
</Callout>
