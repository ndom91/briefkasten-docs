...
| Web + Backend | `BETTER_AUTH_URL`    | Your current URL (i.e. `http://localhost:3000`)                         |
| Web + Backend | `BETTER_AUTH_SECRET` | Set to the output of `openssl rand -hex 33`                             |
| Web           | `PUBLIC_WORKER_URL`  | URL of the `backend` application, (i.e. `http://localhost:8000/api/v0`) |
...
The default object storage provider is Cloudflare R2 as it has a free plan with 10GB storage, no egress fees. But if you'd like to self-host this, I included a [RustFS](https://rustfs.com)-based setup in the `docker-compose.storage.yml` file. You can start it with the following command. Make sure to update the `BUCKET_ACCESS_KEY` and `BUCKET_SECRET_KEY` environment variables in the `docker-compose.storage.yml` file, this will be your bucket access key and secret key, as well as the login for the RustFS console (available in the container at port `9001`).
